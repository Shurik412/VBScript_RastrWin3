...
nowdate = date()
TimeRaschet = nowdate & "_" & HourNow & " hour " & MinuteNow & " minut"
...
rst = 0
if fl = 1 Then
    't.NewFile(Shablon & "режим.rg2")
    't.NewFile(Shablon & "динамика.rst")
    FileRastr = FolderAndMyFile
    FileExcelDynamicSet = FolderAndMyFile
End if 
if fl = 2 then 
    't.NewFile(Shablon & "режим.rg2")
    't.NewFile(Shablon & "динамика.rst")
...
if fl = 1 Then
    if rst = 1 Then 
        t.NewFile(Shablon & "динамика.rst")
        t.Load RG_REPL, FileRastr, Shablon & "динамика.rst"
    Else
        t.NewFile(Shablon & "режим.rg2")
...
VisibelExcelSet = True ' показывать Excel при заполнении 

if fl = 1 Then
    Call Equivalence() ' функция эквивалентирования Корре БРМ
    Call DelNode() ' функция удаления Узлов без ветвей
    FileRastr_2 = FileRastr & "_" & HourNow
    if rst = 0 Then
        t.Save(FileRastr_2 & ".rg2"),(Shablon & "режим.rg2")
        t.NewFile(Shablon & "режим.rg2")
        t.Load RG_REPL, FileRastr_2 & ".rg2", Shablon & "режим.rg2"
        t.Save(FileRastr_2 & ".rst"),(Shablon & "динамика.rst")
        
        t.NewFile(Shablon & "динамика.rst")
        t.Load RG_REPL, FileRastr_2 & ".rst", Shablon & "динамика.rst"
    end if
    Call Populating_Dynamic_Set(FileExcelDynamicSet,VisibelExcelSet,LinkCustomModels)
End if
...
If fl = 2 Then
    ...
    t.Load RG_REPL, FileRastr, Shablon & "режим.rg2"
    Call Equivalence() ' функция эквивалентирования Корре БРМ
    t.Save(FileRastr & "_экв_1"),(Shablon & "режим.rg2")
    t.NewFile(Shablon & "режим.rg2")
    t.Load RG_REPL, FileRastr & "_экв_1", Shablon & "режим.rg2"
    t.rgm "p"
    Call OFF_LEP_one_STA()
    t.rgm "p"
    Call DelNode() ' функция удаления Узлов без ветвей
    t.rgm "p"
    Call DelReactor() 'функция удаления Реакторов без узлов
    t.rgm "p"
    Call DelUSHR() ' функция удаление УШР без узлов
    t.rgm "p"
    Call OffGenP_Q_Zero()
    t.rgm "p"
    t.Save(FileRastr_2 & ".rst"),(Shablon & "динамика.rst")
...
FillingUnspecifiedGen = 1
If FillingUnspecifiedGen = 1 Then 
    t.rgm "p"
    Call FillingUnspecifiedGenerators() ' функция Заполнение незадпнных генераторов  
End if

DelAndChengeDublNumG = 1
If DelAndChengeDublNumG = 1 Then 
    t.rgm "p"
    Call DelAndChengeDublNumGen() ' корректировка дублирующих генераторов (в том числе удаление если Name=Name)
End if 

OFF_LEP_one_ST = 0
If OFF_LEP_one_ST = 1 Then 
    t.rgm "p"
    Call OFF_LEP_one_STA() ' функция откл ЛЭП с односторонним включением 
End if 

OffGenIfNodeSt = 0
If OffGenIfNodeSt = 1 Then 
    t.rgm "p"
    Call OffGenIfNodeSta() ' откл. генераторов с откл. узлами
End if 

OffGenP_Q_Zer = 0
If OffGenP_Q_Zer =1 Then 
    t.rgm "p"
    Call OffGenP_Q_Zero() ' отключение генераторов с Р=0 и Q=0
End if 
...
HourNow2 = Hour(Now())
MinuteNow2 = Minute(Now())
nowdate2 = date()
TimeRaschet2 = nowdate2 & "_" & HourNow2 & " hour " & MinuteNow2 & " minut"

t.Printp("Конец: " & TimeRaschet2)
...
Function DelUSHR()
    ...
    if CountUSHR+1 > 0 Then
        for i = 0 to CountUSHR
            ...
            j = uzl.FindNextSel(-1)
            if j = -1 Then
                'spUSHR.Cols("sel").Z(i) = 1 ' ушр нет sel
                'spUSHR.DelRows 
                ArrayUSHR(ii) = spUSHR.Cols("Id").Z(i)
                ii = ii + 1
            end if
            if j<>-1 Then
                StaNodeUSHR = t.Tables("node").Cols("sta").Z(j)
                if StaNodeUSHR = 1 Then
    ...
    End if
    MaxRowArrayUSHR = LBound(ArrayUSHR)
    for Ar = 0 to MaxRowArrayUSHR
        spUSHR.SetSel("Id=" & ArrayUSHR(Ar))
        spUSHR.DelRows
    next
	t.Printp("Завершение работы функции - удаление УШР без узлов! ( DelUSHR() )")
End Function
...
Function DelReactor()
	...
	ii = spReact.Count-1
	spReact.DelRows
	t.Printp("Завершение работы функции - удаление Реакторов без узлов! ( DelUSHR() )")
End Function
...
Function DelNode()
	...
	t.printp("Количество узлов = " & NodeColMax)
	t.printp("Количество ветвей = " & VetvColMax)
	ii = 0
	...
	t.Printp("Завершение работы функции - удаление Узлов без ветвей! ( DelNode() )")
End Function
...
Function DelAndChengeDublNumGen()
	...
	ii = gen.Count-1
	gen.DelRows
	t.Printp("Было удалено/скорректировано " & ii + ii2 & " генератор(-ов).")
