...
SettingsFile="L:\SecretDisk\SER\Динамическая модель\2020\4.Дин_Модели - 2020 (14.10.2020)\дин_набор (актуальный).xlsm"
...
if ffff=8999 then
	...
	if gener=1 then
		i=3
		while Ex1.cells(i,1).value>0
			if Ex1.cells(i,3).value>0 then
				Eny=Ex1.cells(i,3)
				Eny2=Ex1.cells(i,5)
				...
				if j<>-1 then
					...
					if j2<>-1 then
						numg.Z(j2)=Ex1.cells(i,3)
						nameg.Z(j2)=Ex1.cells(i,4)
						nodeg.Z(j2)=Ex1.cells(i,5)
						nodeg1=Ex1.cells(i,5)
						ModelType.Z(j2)=Ex1.cells(i,6)
						gNumBrand.Z(j2)=Ex1.cells(i,8)
						ExciterId.Z(j2)=Ex1.cells(i,9)
						ARSId.Z(j2)=Ex1.cells(i,10)
						gIVActuatorId.Z(j2)=Ex1.cells(i,11)

						pgpg=Rastr.Calc("val","node","pg","ny="&nodeg1)
						qgqg=Rastr.Calc("val","node","qg","ny="&nodeg1)

						pgen.Z(j2)=pgpg

						qmin.Z(j2)=Rastr.Calc("val","node","qmin","ny="&nodeg1)
						qmax.Z(j2)=Rastr.Calc("val","node","qmax","ny="&nodeg1)
						pnom.Z(j2)=Ex1.cells(i,14)
						unom.Z(j2)=Ex1.cells(i,15)
						cosfi.Z(j2)=Ex1.cells(i,16)
						Demp.Z(j2)=Ex1.cells(i,17)
						mj.Z(j2)=Ex1.cells(i,18)
						xd1.Z(j2)=Ex1.cells(i,19)
						xd.Z(j2)=Ex1.cells(i,20)
						xq.Z(j2)=Ex1.cells(i,21)
						xd2.Z(j2)=Ex1.cells(i,22)
						xq2.Z(j2)=Ex1.cells(i,23)
						td01.Z(j2)=Ex1.cells(i,24)
						td02.Z(j2)=Ex1.cells(i,25)
						tq02.Z(j2)=Ex1.cells(i,26)
						xq1.Z(j2)=Ex1.cells(i,27)
						xl.Z(j2)=Ex1.cells(i,28)
						x2.Z(j2)=Ex1.cells(i,29)
						x2.Z(j2)=Ex1.cells(i,30)
						tq01.Z(j2)=Ex1.cells(i,31)
					end if
				end if
			end if
			i=i+1
	...
	if vozbuzd=1 then
		while Ex2.cells(i,1).value>0
			Eny=Ex2.cells(i,3)
			if Ex2.cells(i,5).value>0 and Ex2.cells(i,3).value>0 then
				vozb.SetSel("")
				vozb.AddRow
				vozb.SetSel(" Id = 0")
				j2=vozb.FindNextSel (-1)
				if j2<>-1 then
					idv.Z(j2)=Ex2.cells(i,3)
					namev.Z(j2)=Ex2.cells(i,4)
					ModelTypev.Z(j2)=Ex2.cells(i,5)
					'Brandv.Z(j2)=Ex2.cells(i,6)
					ExcControlIdv.Z(j2)=Ex2.cells(i,7)
					ForcerIdv.Z(j2)=Ex2.cells(i,8)
	...
	if arv2=1 then
		...
		while Ex3.cells(i,1).value>0
			Eny=Ex3.cells(i,3)
			if Ex3.cells(i,5).value>0 and Ex3.cells(i,3).value>0 then
				arv.SetSel("")
				arv.AddRow
				arv.SetSel("Id = 0")
				j2=arv.FindNextSel (-1)
				if j2<>-1 then
					ida.Z(j2)=Ex3.cells(i,3)
...
if vozbuzdIEEE=1 then
	...
	while Ex5.cells(i,1).value>0
		Eny=Ex5.cells(i,3)
		if Ex5.cells(i,5).value>0 and Ex5.cells(i,3).value>0 then
			vieee.SetSel("")
			vieee.AddRow
			vieee.SetSel("Id = 0")
...
if PSSE2=1 then
	...
	while Ex6.cells(i,1).value>0
		Eny=Ex6.cells(i,3)
		if Ex6.cells(i,5).value>0 and Ex6.cells(i,3).value>0 then
			stieee.SetSel("")
			stieee.AddRow
			stieee.SetSel("Id = 0")
...
if PSS4_2=1 then 
	...
	while Ex7.cells(i,1).value>0
		Eny=Ex7.cells(i,3)
		if Ex7.cells(i,5).value>0 and Ex7.cells(i,3).value>0 then
			pss4.SetSel("")
			pss4.AddRow
			pss4.SetSel("Id = 0")
...
if turbina=1 then
	...
	while Ex4.cells(i,1).value>0
		Eny=Ex4.cells(i,3)
		if Ex4.cells(i,5).value>0 and Ex4.cells(i,3).value>0 then
			ars.SetSel("")
			ars.AddRow
			ars.SetSel("Id = 0")
			...
			if j2<>-1 then
				...
				arsGovernorId.Z(j2)=Ex4.cells(i,7)
...
'----------------АРС----------------
	...
	while Ex12.cells(i,1).value>0
		'Eny=Ex12.cells(i,3)
		if Ex12.cells(i,5).value>0 and Ex12.cells(i,3).value>0 then
			Governor.SetSel("")
			Governor.AddRow
			...
			if j2<>-1 then
				...
				GovernorId.Z(j2)=Ex12.cells(i,3)
...
if forc2=1 then
	...
	while Ex8.cells(i,1).value>0
		Eny=Ex8.cells(i,3)
		if Ex8.cells(i,5).value>0 and Ex8.cells(i,3).value>0 then
			forc.SetSel("")
			forc.AddRow
			forc.SetSel("Id = 0")
			j2=forc.FindNextSel (-1)
			if j2<>-1 then
				idf.Z(j2)=Ex8.cells(i,3)
...
if DECS_400=1 then 
	...
	while Ex13.cells(i,1).value>0
		Eny=Ex13.cells(i,3)
		if Ex13.cells(i,5).value>0 and Ex13.cells(i,3).value>0 then
			decs400.SetSel("")
			decs400.AddRow
			decs400.SetSel("Id = 0")
...
if Thyne4=1 then 
	...
	while Ex14.cells(i,1).value>0
		Eny=Ex14.cells(i,3)
		if Ex14.cells(i,5).value>0 and Ex14.cells(i,3).value>0 then
			Thyne.SetSel("")
			Thyne.AddRow
			Thyne.SetSel("Id = 0")
...
'--------------цены делений---------------------
On_ExcControlParam = 1
if On_ExcControlParam = 1 then 
